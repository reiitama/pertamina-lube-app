        ...
        <div class="card">
            <div class="card-body">
                <div class="dataTables_length">
                    ...
                    <button id="exportSelectedExcel" class="btn btn-primary float-right ml-2">
                ...
                <table id="dataTable" class="table table-striped table-bordered" style="width:100%">
                    ...
                    @foreach ($result as $row)
                        <tr>
                            <td>
                                <input type="checkbox" id="selectedRow_{{ $row->condemID }}" name="selectedRows[]"
                                       value="{{ $row->condemID }}">
                            </td>
                            <td>{{ $row->manufac }}</td>
                            <td>{{ $row->compoName }}</td>
                            ...
                            <td class="align-middle">
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    <button type="button" class="btn btn-secondary detail-button"
                                            data-condem-id="{{ $row->condemID }}">Detail
                                    </button>
                                    <a href="{{ route('oil.edit', $row->condemID) }}" type="button"
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.detail-button').on('click', function () {
                ...
                $.ajax({
                    url: "{{ route('oil.show', ':id') }}".replace(':id',
                        condemId), // Mengganti :id dengan condemId
                    type: 'GET',
                    ...
                    error: function () {
                        alert('Failed to load condemning limit detail.');
                    }
                });
            });
            ...
            $('#exportSelectedExcel').on('click', function () {
                ...
                const exportExcelUrl = "{{ route('oil.export.excel', '') }}" + '/' + selectedRowIds.join(',');
                var link = document.createElement('a');
